...
export function CountdownBanner({
  ...
  return (
    <div className="bg-[#111827] border-b border-blue-900/50 py-3 sm:py-4 px-4 sm:px-6 relative overflow-hidden flex items-center min-h-[80px]">
      <div className="max-w-7xl mx-auto w-full flex flex-col lg:flex-row items-center justify-between gap-4 lg:gap-8">
        ...
        <div className="flex items-center gap-3 sm:gap-4 w-full lg:w-auto">
          ...
          <div className="flex flex-col">
            <h3 className="text-white font-bold text-sm sm:text-base leading-tight flex flex-wrap gap-1">
              {headline} <span className="text-blue-400">— {highlightText}</span>
            </h3>
            {subtitle && (
              <p className="text-gray-400 text-xs sm:text-sm mt-0.5 font-medium hidden sm:block">
                {subtitle}
              </p>
            )}
          </div>
        ...
        <div className="flex items-center gap-3 sm:gap-6 w-full lg:w-auto justify-between lg:justify-end">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <span className="text-gray-400 text-xs sm:text-sm font-medium hidden md:block whitespace-nowrap">
          ...
          <button 
            onClick={scrollToCTA}
            className="flex items-center underline gap-1.5 text-white text-xs sm:text-sm font-bold hover:text-blue-400 transition-colors shrink-0 group"
          >
            {buttonText} 
...
function TimeBlock({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex items-baseline gap-0.5 bg-[#1e293b] border border-blue-500/20 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded sm:rounded-md shadow-[0_0_10px_rgba(59,130,246,0.1)]">
      <span className="text-sm sm:text-base tracking-widest">{value}</span>
      <span className="text-[10px] sm:text-xs text-blue-400 font-medium">{label}</span>
    </div>
  );
}
