...
import { Check, ArrowRight, Zap, Code, Database, Layout, Smartphone, Globe, LineChart, TrendingUp } from 'lucide-react';
    ...
    <div className="pt-32 min-h-screen bg-white font-sans relative overflow-hidden selection:bg-zeven-blue selection:text-white">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        {processSteps && processSteps.length > 0 && (
          <div className="mb-32">
            <ScrollReveal>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-white/40 backdrop-blur-xl border border-white/60 p-4 rounded-[2rem] shadow-sm">
                {processSteps.map((step: any, idx: number) => (
                  <div key={idx} className="relative h-full group p-6 rounded-2xl bg-zeven-blue border border-white/10 shadow-lg hover:shadow-zeven-blue/40 hover:-translate-y-2 transition-all duration-300 overflow-hidden">
                    {/* Gradient Overlay */}
        ...
        <div className="mb-24 md:mb-40">
          <ScrollReveal>
            <div className="text-center mb-20 max-w-3xl mx-auto">
              <h2 className="text-4xl md:text-5xl font-extrabold text-zeven-dark mb-6 tracking-tight" dangerouslySetInnerHTML={{ __html: servicesPageData?.pillarsHeading?.heading || 'Built on Three Core Pillars' }}>
              </h2>
              <p className="text-lg text-zeven-gray font-light leading-relaxed">
                {servicesPageData?.pillarsHeading?.description || 'Our comprehensive approach combines creative excellence, technical expertise, and strategic marketing to deliver complete digital solutions.'}
              </p>
            </div>
          </ScrollReveal>
          ...
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
            {pillars.map((pillar: any, i: number) => {
              const icons = [Layout, Globe, TrendingUp];
              ...
              return (
                <ScrollReveal key={i} delay={i * 0.15}>
                  <div className="h-full bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] p-10 flex flex-col items-center text-center group hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] transition-all duration-500 hover:-translate-y-2">
                    {/* Icon Container */}
                    ...
                    <div className="w-full pt-8 border-t border-slate-100 mt-auto">
                      <ul className="flex flex-col gap-3 text-left">
                        {(pillar.capabilities || []).map((cap: string, ci: number) => (
                          <li key={ci} className="flex items-center gap-3 text-zeven-dark text-sm font-medium">
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-zeven-dark mb-4" dangerouslySetInnerHTML={{ __html: servicesPageData?.servicesGridHeading?.heading || 'Our Core Services' }}></h2>
          {servicesPageData?.servicesGridHeading?.description && (
            <p className="text-xl text-zeven-gray max-w-2xl mx-auto font-light">{servicesPageData.servicesGridHeading.description}</p>
          )}
        </div>
          ...
          <div className="mb-20 rounded-[3rem] p-12 md:p-24 text-center relative overflow-hidden isolate shadow-2xl">
            ...
            <div className="relative z-10">
              ...
              <Link to={servicesPageData?.notSureWhereToStartCta?.button?.url || "/contact"}>
                {/* <Button size="lg" className="bg-white text-zeven-dark  border-none  text-lg px-12 py-5 h-auto rounded-full font-bold">
                </Button> */}
                <button className="bg-white text-zeven-blueDark px-6 sm:px-12 py-5 rounded-full font-bold text-xl hover:scale-105 transition duration-300 shadow-2xl shadow-black/20">
